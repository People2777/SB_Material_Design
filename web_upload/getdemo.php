...
if (!isset($_GET['id']) || !isset($_GET['type']))
    die('Нет ID демо-записи/типа.');

if (strcasecmp($_GET['type'], "U") != 0 && strcasecmp($_GET['type'], "B") != 0 && strcasecmp($_GET['type'], "S") != 0)
    die('Плохой тип демо-записи.');

$demo = $GLOBALS['db']->getRow("SELECT filename, origname FROM `:prefix:demos` WHERE demtype=? AND demid=?;", array($_GET['type'], intval($id)));

if (!$demo)
    die('Демо-запись не найдена.');
